...
function prompt_start() {
  ...
  local ret_status="%(?:%{$fg[green]%} $ICONS[ubuntu_logo] :%{$fg[red]%} $ICONS[ubuntu_logo] )${reset_color}"
...
#   local node_icon="%{$fg[green]%}${ICONS[node]}"
#   if [ -f .ruby-version ] ; then
#     local version="%{$fg[white]%}$(cat .ruby-version)"
#     echo "%{$FG[248]%}$(rprompt_separator) %{$BG[248]%} ${node_icon} ${version}"
#   fi
# }
