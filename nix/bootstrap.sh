...
#
#   bash ~/dotfiles/nix/bootstrap.sh            # auto-detects arch vs arch-wsl
#   HOST=ubuntu bash ~/dotfiles/nix/bootstrap.sh
#
# HOST must match an entry in flake.nix `homeConfigurations` (arch | arch-wsl |
# ubuntu | macbook). When unset it defaults to arch-wsl under WSL, else arch.
...
# Default host: WSL Arch -> arch-wsl, otherwise bare-metal arch. Override with
# HOST=<arch|arch-wsl|ubuntu|macbook>.
if [ -z "${HOST:-}" ]; then
...
if [ "${SKIP_MISE:-0}" != "1" ]; then
  ...
  if [ -n "$mise_bin" ]; then
    ...
    # Register engram (brew / pi extension) with each detected coding agent.
